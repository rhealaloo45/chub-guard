...
This document illustrates the stark difference in the developer experience and project safety when using the **chub Deprecation Guard** versus a standard Python development workflow.

---

## 1. The Standard Workflow (Without the Guard)

In a standard workflow, AI agents or developers might copy/paste outdated SDK code. Standard linters (`ruff`, `flake8`) do not know about third-party API deprecations, so the bad code successfully enters the repository, leading to runtime failures or technical debt.

```mermaid
graph TD
    A[Developer writes code<br/>Uses AI/Docs] -->|Code contains outdated SDK API| B(git commit)
    B --> C{Standard Linters<br/>e.g., ruff, mypy}
    C -->|Passes ✅<br/>Valid Python syntax| D[Code merged into repository]
...
1. **Copy Files:** Copy `scripts/chub_guard.py` and `.chub-docs/registry.json` into the target project.
2. **Add Config:** Add the hook definition to `.pre-commit-config.yaml`.
3. **Install:** Run `pre-commit install` in the terminal.
...
graph TD
    ...
    C -->|1. Parse AST| D[Identify AI SDKs used]
...
To scan all Python files in the current repository without triggering a commit, you can use `pre-commit`'s manual run command:

```bash
# Scans every .py file in the repository
pre-commit run chub-deprecation-guard --all-files
```

### Auditing Specific Files (Direct Python Execution)
...
```bash
# Scan specific files
python scripts/chub_guard.py run src/main.py src/llm_client.py

# Scan a whole directory (Linux/macOS)
python scripts/chub_guard.py run $(find src/ -name "*.py")

# Scan a whole directory (Windows PowerShell)
python scripts/chub_guard.py run (Get-ChildItem -Path src\ -Recurse -Filter *.py).FullName
```
